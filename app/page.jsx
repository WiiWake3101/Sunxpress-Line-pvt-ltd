...
/* ─── Data ─── */
const services = [
  { icon: "🚢", title: "Ocean Freight",     desc: "FCL & LCL solutions across major global trade lanes with real-time cargo tracking.", accent: "#437A96" },
  { icon: "⚓", title: "Bulk Cargo",        desc: "Efficient handling of coal, grain, fertilizers and minerals with dedicated bulk terminals.", accent: "#319795" },
  { icon: "🏭", title: "Port Logistics",    desc: "End-to-end port operations and stevedoring at Tuticorin — India's southern gateway.", accent: "#2C5282" },
  ...
  { icon: "🗺️", title: "Route Planning",   desc: "Strategic routing across Indian Ocean trade lanes optimised for cost and safety.", accent: "#2C5282" },
];

const stats = [
  { value: "50+",  label: "Ports Served", icon: "⚓" },
  { value: "200+", label: "Shipments",    icon: "📦" },
  { value: "15+",  label: "Trade Routes", icon: "🗺️" },
  { value: "100%", label: "On-Time Rate", icon: "✅" },
];

const fleet = [
  { name: "Bulk Carrier",     type: "Heavy Cargo",   capacity: "25,000 DWT", color: "#1A365D" },
  { name: "Container Vessel", type: "FCL / LCL",     capacity: "1,200 TEU",  color: "#2C5282" },
  { name: "Multi-Purpose",    type: "General Cargo", capacity: "12,000 DWT", color: "#319795" },
  { name: "Coastal Tanker",   type: "Liquid Bulk",   capacity: "8,500 DWT",  color: "#437A96" },
];
    ...
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row items-start gap-12 py-24 lg:py-32">
            ...
            <div className="flex-1 text-white pt-4">
              ...
              {/* Stats row */}
              <div className="flex flex-wrap gap-0 rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.1)", animation: "fadeSlideUp 0.9s ease 0.4s both" }}>
                {stats.map((s, i) => (
                  <div key={s.label} className="flex-1 min-w-[100px] px-5 py-4 text-center"
                    style={{ background: i % 2 === 0 ? "rgba(255,255,255,0.07)" : "rgba(255,255,255,0.04)", borderRight: i < stats.length - 1 ? "1px solid rgba(255,255,255,0.08)" : "none" }}>
      ...
      <section id="fleet" className="w-full py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 gap-6">
            <Reveal direction="left">
              <span className="text-xs font-black tracking-widest uppercase" style={{ color: "#319795" }}>— Our Fleet</span>
              <h2 className="text-4xl sm:text-5xl font-black mt-3" style={{ color: "#0D1F3C", fontFamily: "'Georgia', serif", letterSpacing: "-0.02em" }}>Vessels for<br />Every Cargo</h2>
            </Reveal>
            <Reveal direction="right">
              <p className="text-base max-w-xs" style={{ color: "#718096", lineHeight: "1.8" }}>A versatile fleet engineered for bulk, containerised, and liquid cargo with equal reliability.</p>
            </Reveal>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {fleet.map((f, i) => (
              <Reveal key={f.name} delay={i * 100} direction="up">
                <div className="rounded-2xl overflow-hidden group transition-all duration-300 hover:-translate-y-1 hover:shadow-xl" style={{ border: "1.5px solid #EDF2F7", boxShadow: "0 4px 20px rgba(0,0,0,0.05)" }}>
                  <div className="h-36 flex items-center justify-center relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${f.color}, ${f.color}CC)` }}>
                  ...
                  <div className="p-5 bg-white">
                    <span className="text-xs font-black tracking-widest uppercase" style={{ color: f.color }}>{f.type}</span>
                    <h3 className="text-base font-bold mt-1 mb-2" style={{ color: "#1A365D" }}>{f.name}</h3>
                    <div className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full" style={{ background: "#F0F4F8", color: "#4A5568" }}>⚓ {f.capacity}</div>
                  </div>
                </div>
              </Reveal>
            ))}
                ...
                <div className="flex flex-col gap-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    {[{ id: "name", label: "Full Name", placeholder: "John Smith" }, { id: "company", label: "Company", placeholder: "Your Company Ltd." }].map((f) => (
                      <div key={f.id}>
                        ...
                        <input type="text" placeholder={f.placeholder} className="w-full px-4 py-3.5 text-sm rounded-xl outline-none transition-all"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    {[{ id: "email", label: "Email", placeholder: "you@company.com", type: "email" }, { id: "phone", label: "Phone", placeholder: "+91 XXXXX XXXXX", type: "tel" }].map((f) => (
                      <div key={f.id}>
                        <label className="block text-xs font-bold mb-2 tracking-wide" style={{ color: "#4A5568" }}>{f.label}</label>
                        <input type={f.type} placeholder={f.placeholder} className="w-full px-4 py-3.5 text-sm rounded-xl outline-none transition-all"
                          style={{ border: "1.5px solid #E2E8F0", color: "#2D3748", background: "#FAFBFC" }}
                          onFocus={(e) => { e.target.style.borderColor = "#319795"; e.target.style.boxShadow = "0 0 0 3px rgba(49,151,149,0.1)"; }}
                          onBlur={(e)  => { e.target.style.borderColor = "#E2E8F0"; e.target.style.boxShadow = "none"; }} />
                      </div>
                  ...
                  <div>
                    ...
                    <select className="w-full px-4 py-3.5 text-sm rounded-xl outline-none" style={{ border: "1.5px solid #E2E8F0", color: "#2D3748", background: "#FAFBFC" }}>
                  ...
                  <div>
                    ...
                    <textarea rows={4} placeholder="Describe your cargo, origin port, destination port, and any special requirements…"
